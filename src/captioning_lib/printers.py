    ...
    def print(self, transcript, duration=None, partial=False, is_recent_chunk_mode=False, recent_chunk_duration=None):
        ...
        if partial:
            if self.verbose and is_recent_chunk_mode and recent_chunk_duration:
                print(f"\r\033[2K\r1PARTIAL (recent-chunk, {recent_chunk_duration:.1f}s chunk/{duration:.1f}s total): {transcript}", flush=True, end='')
                time.sleep(VERBOSE_STREAMING_DELAY)
            elif self.verbose and duration:
                print(f"\r\033[2K\r1PARTIAL (retranscribe, {duration:.1f}s total): {transcript}", flush=True, end='')
                time.sleep(VERBOSE_STREAMING_DELAY)
            #else:
               # print(f"\r\033[2K\r1PARTIAL: {transcript}", flush=True, end='')
        else: 
            if self.verbose and duration:
                print(f"\rVP1:SEGMENT ({duration:.1f}s total): {transcript}")
            else:
                print(f"\rP1:SEGMENT BEFORE LLAMMA: {transcript}")
                ...
                # - DO NOT change if sentense already correct
                # """
